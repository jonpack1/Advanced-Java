...
public class Diagonal {
        ...
        private static boolean isValidPlacement(int[][] board, int number, int row, int column) {
            return !isNumberInRow(board, number, row) &&
                    ...
                    !isNumberInDiagonal(board,number,row,column);
        }

        public static boolean isNumberInDiagonal(int[][] board, int number, int row, int column){
            // Check if ch is already present in the first diagonal
            if (row == column) {
                for (int i = 0; i < GRID_SIZE; i++) {
                    if (board[i][i] == number && (i != row || i != number)) {
                        return true;
                    }
                }
            }
            // Check if ch is already present in the other diagonal
            if (row + column == GRID_SIZE - 1) {
                for (int i = 0; i < GRID_SIZE; i++) {
                    if (board[i][GRID_SIZE - 1 - i] == number && (i != row || GRID_SIZE - 1 - i != column)) {
                        return true;
                    }
                }
            }
